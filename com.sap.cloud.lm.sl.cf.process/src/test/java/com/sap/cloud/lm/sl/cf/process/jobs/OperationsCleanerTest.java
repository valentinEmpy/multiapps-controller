...
public class OperationsCleanerTest {
    ...
    private void initQueryMockForPage(int pageIndex, List<Operation> result) {
        OperationQuery queryMock = new MockBuilder<>(operationQuery, OperationQuery.class).on(OperationQuery::inNonFinalState)
                                                                                          .on(query -> query.startedBefore(EXPIRATION_TIME))
                                                                                          .on(query -> query.offsetOnSelect(pageIndex
                                                                                              * PAGE_SIZE))
    ...
    public void testExpiredOperationsAreDeleted() {
        when(operationService.createQuery()).thenReturn(operationQuery);
        OperationQuery mock = new MockBuilder<>(operationQuery, OperationQuery.class).on(query -> query.startedBefore(EXPIRATION_TIME))
                                                                                     .build();
        cleaner.execute(EXPIRATION_TIME);
        verify(mock).delete();
    }
