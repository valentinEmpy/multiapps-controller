...
public class ProgressMessageDto implements DtoWithPrimaryKey<Long> {
    ...
    public ProgressMessage toProgressMessage() {
        return new ProgressMessage(id, processId, taskId, getParsedType(type), text, timestamp);
    }
    ...
    private void validateState() {
        for (Entry<String, String> field : getNonNullableFields().entrySet()) {
            if (field.getValue() == null) {
                throw new IllegalStateException(
                    MessageFormat.format(Messages.PROGRESS_MESSAGE_COLUMN_VALUE_SHOULD_NOT_BE_NULL, field.getKey()));
            }
        }
    }

    private Map<String, String> getNonNullableFields() {
        Map<String, String> fields = new TreeMap<>();
        fields.put(TableColumnNames.PROGRESS_MESSAGE_PROCESS_ID, processId);
        fields.put(TableColumnNames.PROGRESS_MESSAGE_TASK_ID, taskId);
        fields.put(TableColumnNames.PROGRESS_MESSAGE_TYPE, type);
        fields.put(TableColumnNames.PROGRESS_MESSAGE_TEXT, text);
        return fields;
    }
}
