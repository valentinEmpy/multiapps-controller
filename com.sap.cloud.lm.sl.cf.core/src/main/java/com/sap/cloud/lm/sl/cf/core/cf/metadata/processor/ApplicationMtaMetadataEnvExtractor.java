...
import com.sap.cloud.lm.sl.cf.core.cf.metadata.ImmutableApplicationMtaMetadata;
...
public class ApplicationMtaMetadataEnvExtractor {

    private static final Logger LOGGER = LoggerFactory.getLogger(ApplicationMtaMetadataEnvExtractor.class);

    private ApplicationMtaMetadataEnvExtractor() {
    }

    public static ApplicationMtaMetadata extractMetadata(CloudApplication app) {
        try {
            return attemptToExtractMetadata(app);
        } catch (ParsingException e) {
            throw new ParsingException(e, Messages.CANT_PARSE_MTA_ENV_METADATA_FOR_APP_0, app.getName());
        }
    }

    private static ApplicationMtaMetadata attemptToExtractMetadata(CloudApplication app) {
        ...
        DeployedMtaModule module = DeployedMtaModule.builder()
                                                    ...
                                                    .build();
        return ImmutableApplicationMtaMetadata.builder()
                                              .mtaMetadata(mtaMetadata)
                                              .deployedMtaModule(module)
                                              .build();
    }
