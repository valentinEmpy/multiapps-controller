...
public class ProgressMessageDtoDao extends AbstractDtoDao<ProgressMessageDto, Long> {
    ...
    protected NotFoundException getNotFoundException(Long id) {
        return new NotFoundException(Messages.PROGRESS_MESSAGE_NOT_FOUND, id);
    }

    @Override
    protected ConflictException getConflictException(ProgressMessageDto progressMessage) {
        return new ConflictException(Messages.PROGRESS_MESSAGE_ALREADY_EXISTS, progressMessage.getProcessId(),
